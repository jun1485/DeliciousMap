...
#category li .category_bg {background:url(places_category.v2.png) no-repeat;}
#category li .restaurant {background-position: -10px 0;}
#category li .cafe {background-position: -10px -36px;}
#category li .convenience {background-position: -10px -72px;}
#category li .mart {background-position: -10px -108px;}
#category li .tour {background-position: -10px -144px;}
#category li .accommodation {background-position: -10px -180px;}
    ...
    <header class="header nav-header">
        ...
        <ul id="category">
            ...
            <li id="CS2" data-order="2"> 
                <span class="category_bg convenience"></span>
                편의점
            </li>
...
for(int i =0; i < s.size(); i++){
	...
	for(int j = 0; j < schedule.getElementCount(); j++){
        ...
        tmp += "<td><div class='Etitle'>"+title+"<br><div class='Escore'>★4.5</div></div></td><td class='bar' rowspan='2'> </td></tr>";
...
function displayPlaces(places) {
    ...
    for(var i=0;i<places.length;i++){
        ...
        (function(marker, title) {
            ...
            //마커를 클릭 시 popup창을 열기
            // kakao.maps.event.addListener(marker, 'click', function() {
            //     open_pop(title, marker.getPosition().getLat(), marker.getPosition().getLng());
            // });
...
function open_pop(title, lat, lng){
    window.name = "schedule.jsp";
    var url = 'popup.jsp?title='+ title + "&lat=" + lat + "&lng=" + lng + "&dayCount=" + dayCount;		// jsp가 받을 수 있도록 형식 변경
...
function add_schedule(start, end, display_time, title){
	...
	//오버레이 실행
    function RunOverlay(title, lat, lng){
        CloseOverlay(); //기존 오버레이를 초기화
        CtremoveMarker(); //카테고리 마커 지우기
        var tmp = '<div class="customOverlay"><div class="Title_customOverlay">'+title+'</div><div class="img_customOverlay">img</div>';
        tmp += '<div class="Btns"><button onclick="open_pop('+title+','+lat+','+lng+')" class="ABtn">추가</button><button onclick="CloseOverlay()" class="CBtn">닫기</button></div></div>';
...
<script>
    ...
    function CtsearchPlaces() {
        ...
        // 지도에 표시되고 있는 마커를 제거합니다
        CtremoveMarker();
        Ctps.categorySearch(currCategory, CtplacesSearchCB, {useMapBounds:true}); 
    }
    ...
    function CtdisplayPlaces(places) {
        ...
        for ( var i=0; i<places.length; i++ ) {
                // 마커를 생성하고 지도에 표시합니다
                var marker = CtaddMarker(new kakao.maps.LatLng(places[i].y, places[i].x), order);
                // 마커와 검색결과 항목을 클릭 했을 때
                // 장소정보를 표출하도록 클릭 이벤트를 등록합니다
                (function(marker, place) {
                    kakao.maps.event.addListener(marker, 'click', function() {
                        RunCrtOverlay(place, marker.getPosition().getLat(), marker.getPosition().getLng());
                    });
                })(marker, places[i]);
        }
    }
    ...
    function RunCrtOverlay(place, lat, lng){
        CloseOverlay(); //일반 오버레이 지우기
        CtremoveMarker(); //카테고리 오버레이 지우기
        var name = place.place_name, id = place.id;
        var tmp = '<div class="customOverlay" style="background-color:firebrick;">'+
            '<div class="Title_customOverlay">'+name+'</div>'+
            '<div class="img_customOverlay">img</div>';
        tmp += '<div class="Btns"><button onclick="open_detail(&quot;'+id+'&quot;,&quot;'+name+'&quot;,'+lat+','+lng+')" class="detail">상세보기</button>'+
            '<button onclick="#" class="ABtn">즐겨찾기</button><button onclick="CloseOverlay()" class="CBtn">닫기</button></div></div>';
        customOverlay = new kakao.maps.CustomOverlay({
        map: map,
        clickable: true,
        content: tmp,
